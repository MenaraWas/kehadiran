    ...
    <script>
        Alpine.data('ocrCamera', () => ({
            ...
            loadTesseract() {
                ...
                s.async = true;
                s.onload = () => { this.tesseractReady = true; };
            ...
            async startCamera() {
                ...
                if (!window.isSecureContext) {
                    this.cameraError = 'Kamera memerlukan koneksi HTTPS. Gunakan opsi "Upload Foto Surat" sebagai alternatif.';
                    return;
                }
                ...
                try {
                    ...
                    if (err.name === 'NotAllowedError') {
                        this.cameraError = 'Izin kamera ditolak. Aktifkan izin kamera di pengaturan browser, atau gunakan "Upload Foto Surat".';
                    } else if (err.name === 'NotFoundError') {
                        this.cameraError = 'Kamera tidak ditemukan. Gunakan "Upload Foto Surat" sebagai alternatif.';
                    } else if (err.name === 'NotReadableError') {
                        this.cameraError = 'Kamera sedang digunakan aplikasi lain. Gunakan "Upload Foto Surat".';
                    } else {
                        this.cameraError = 'Gagal mengakses kamera: ' + (err.message || err.name) + '. Gunakan "Upload Foto Surat".';
                    }
                }
            },
            ...
            capturePhoto() {
                ...
                if (!video || !canvas) {
                    this.cameraError = 'Elemen video/canvas tidak ditemukan.';
                    return;
                }
            ...
            async startOcr() {
                ...
                if (!this.tesseractReady || !window.Tesseract) {
                    this.loadTesseract();
                    alert('Pustaka OCR sedang dimuat. Silakan coba lagi dalam beberapa detik.');
                    return;
                }
                ...
                try {
                    ...
                    if (!text) {
                        this.detectedText = '(Tidak ada teks terdeteksi)';
                        return;
                    }
                    ...
                    alert('Gagal melakukan scan OCR: ' + (err.message || 'Error'));
                } finally {
                    this.isScanning = false;
                }
            },
            ...
            analyzeText(text) {
                ...
                if (/sakit|dokter|medis|kesehatan|opname|rumah\s*sakit/i.test(text)) {
                    matchedStatus = 'Sakit';
                } else if (/izin|memohon|keperluan|halangan|pernikahan|keluarga/i.test(text)) {
                    matchedStatus = 'Izin';
                } else if (/tugas|perintah|dinas|spt\b|sppd|pelatihan/i.test(text)) {
